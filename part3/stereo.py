...
# Ref: http://nghiaho.com/?page_id=1366
...
def transition_cost(disp_costs, alpha = 300):
    ...
    between labels (D^2)
    """
    # shape of v matrix is (H,W,D,C) where
    # D : the number of label
    # C : Combination of transition between label with the same shape as D
...
def stereoscopic_image(img1,img2):
    ...
    new_img = Image.merge('RGB',[left_img.getchannel('R'),right_img.getchannel('G'),right_img.getchannel('B')])
...
if __name__ == "__main__":
    ...
    disp_costs = disparity_costs(image1, image2, size=5, alpha=0.53) #alpha=0.53, 0.95,
    ...
    v_cost = transition_cost(disp_costs,alpha=100) #alpha = 300, 100, greater alpha more sensitive the image is
    disp3 = mrf_stereo(disp_costs, v_cost,iter=5, beta=1) # beta = 0.5, 1.5
